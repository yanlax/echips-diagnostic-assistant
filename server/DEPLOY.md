...
## Резервная копия
Функция `echips-backup` (код `server/backup.py`) по таймеру каждую ночь (00:00 UTC = 03:00 по Москве, триггер `echips-backup-daily`)
копирует всё новое и изменённое из основного бакета в бакет `echips-backup-1849ef5e`. Ничего не удаляет; в обоих бакетах включено
версионирование, поэтому перезаписанные файлы (список инженеров, профили) хранятся в прошлых версиях. Запустить вручную:
`yc serverless function invoke --name echips-backup`. Восстановить: скопировать нужный ключ из бакета копии обратно.
