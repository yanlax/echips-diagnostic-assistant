...
pub fn diagnose(entries: &[CrashEntry], hw: &HwCounts) -> Vec<Diagnosis> {
    ...
    if power >= 2 || (power >= 1 && n >= 1) {
        push(
            ...
            "Питание (батарея, зарядка/БП), перегрев или плата — либо удержание кнопки питания (в т. ч. при тестировании): Windows не различает эти случаи. Прогоните без батареи и с другим блоком питания, посмотрите температуры под нагрузкой.",
            &["bat", "sens", "stress"],
        );
    }
